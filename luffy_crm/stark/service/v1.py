...
def get_time_text(title, field, date_formate='%Y-%m-%d'):
    """
    对于Stark组件中定义列时，choice如果想要显示中文信息，调用此方法即可。
    :param title: 希望页面显示的表头
    :param field: 字段名称
    ...
    def inner(self, obj=None, is_header=None, *args, **kwargs):
        ...
        return date_value.strftime(date_formate)
...
def get_m2m_text(title, field):
    def inner(self, obj=None, is_header=None, *args, **kwargs):
        if is_header:
            return title
        date_queryset = getattr(obj, field).all()
        date_list = [str(row) for row in date_queryset]
...
class StarkHandler(object):
    ...
    def add_view(self, request, *args, **kwargs):
        ...
        :return:
        """
        model_form_class = self.get_model_form_class('add', request, None, *args, **kwargs)
    ...
    def change_view(self, request, pk, *args, **kwargs):
        ...
        :return:
        """
        current_change_object = self.get_change_obj(request, pk, *args, **kwargs)
        if not current_change_object:
            return HttpResponse('要修改的数据不存在，请重新选择！')

        model_form_class = self.get_model_form_class('change', request, pk, *args, **kwargs)
        if request.method == 'GET':
    ...
    def get_list_url_name(self, *args, **kwargs):
        ...
        :return:
        """
        return self.get_url_name('list', *args, **kwargs)
    ...
    def reverse_common_url(self, addr_name, *args, **kwargs):
        name = "%s:%s" % (self.site.namespace, addr_name)
